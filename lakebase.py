...
def _bootstrap():
    ...
    with _tables_lock:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"CREATE INDEX IF NOT EXISTS permissions_email_idx "
                f"ON {PERMISSIONS} (lower(email))"
            )
        _tables_ready = True
...
def _ensure_permissions_ready():
    """Bootstrap the table and, one time per worker, backfill it from the warehouse
    if it is still empty — so the cutover is transparent and rollback-safe."""
    ...
    with _tables_lock:
        if _perms_backfilled:
            return
        rows = pg_query(f"SELECT count(*) AS n FROM {PERMISSIONS}")
...
def _backfill_permissions():
    """One-time seed of Lakebase permissions from the warehouse table."""
    ...
    for r in src:
        pg_execute(
            ...
            f"VALUES (%s, %s, %s, COALESCE(%s::timestamptz, NOW()))",
